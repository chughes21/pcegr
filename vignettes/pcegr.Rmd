...
plot(pceg1)
```
There are two graphs plotted, but only the final PCEG on the right hand side should be considered. On the left hand side is an intermediate plot used for propagation analysis, and while not particularly useful, is a legacy of the plotting method for the S4 class.
...
```{r}
total_value_extractor(knee_pain_obs,ptree,level_exclude = 0, zip=FALSE,dec_place=2)
value_extractor(knee_pain_obs,ptree,level_rel_final = 0,true_value = pceg_lambda, zip=FALSE,dec_place=2)
```
...
chi_sq_calculator(knee_pain_obs,mapmod,zip=TRUE)$chi_sq-chi_sq_calculator(knee_pain_obs,zipmod2,zip=TRUE)$chi_sq
```
