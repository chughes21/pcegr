...
```
For Age, we can see a horizontal line for both cases when weight is Over. This represents the conditional independence of knee pain and age given that the individual is overweight. For each variable, besides this conditional independence, we can see that the marginal effect decreases when the value changes from 0 to 1. That is, being Young, being a Normal weight, and having No history of injury all decrease your rates of knee pain. In the plot for history, we can only see three lines. This is because the two lines for Overweight are the exact same, once again representing the conditional independence from Age given Overweight.
...
```{r}
marginal_effect(knee_pain_obs,ptree,output_variable=3)
```
...
```{r}
marginal_effect(knee_pain_obs,ziptree2,output_variable=4,zip=TRUE)
```
Instead, we focus on the risk probabilities. 
