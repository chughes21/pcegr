...
```
For Age, we can see a horizontal line for both cases when weight is Over. This represents the conditional independence of knee pain from age given that the individual is overweight. For each variable, besides this conditional independence, we can see that the marginal effect decreases when the value changes from 0 to 1. That is, being Young, being a Normal weight, or having No history of injury all decrease your rates of knee pain. In the plot for history, we can only see three lines. This is because the two lines for Overweight are the exact same (the green endpoints can be seen with the purple line), once again representing the conditional independence from Age given Overweight.
...
```{r}
ptree$model.score
chi_sq_calculator(knee_pain_obs,limit=3,ptree,zip=FALSE,dec_place=2)
```
...
```{r}
set.seed(13579)
ziptree1<-zipceg(knee_pain_obs,"Gibbs",iter=1000,variable_time = TRUE, stoch_imputation = TRUE, gamma_alpha = a, gamma_beta = b, beta_c = c, beta_d = d)
summary(ziptree1)
...
set.seed(13579)
ziptree2<-zipceg(knee_pain_obs,"Gibbs",iter=1000,variable_time = TRUE, remove_risk_free=TRUE, stoch_imputation = TRUE, gamma_alpha = a, gamma_beta = b, beta_c = c, beta_d = d)
summary(ziptree2)
plot(ziptree2)
```
...
```
We can see that a Young age, a Normal weight, or No history all increase the probability of being risk free, as expected. 
...
```{r}
set.seed(2468)
ziptree3<-zipceg(knee_pain_obs,"nlm",variable_time = TRUE, stoch_imputation = TRUE, gamma_alpha = a, gamma_beta = b)
plot(ziptree3)
...
chi_sq_calculator(knee_pain_obs,ziptree1,limit=3)$chi_sq-chi_sq_calculator(knee_pain_obs,ziptree3,limit=3)$chi_sq
```
...
```{r}
set.seed(1098765)
maptree<-zipceg.iter(knee_pain_obs,"nlm",iter_total=100,variable_time = TRUE,remove_risk_free = TRUE, gamma_alpha=a,gamma_beta=b,print_output = FALSE)$mod
set.seed(314159)
noprint.mod<-zipceg.iter(knee_pain_obs,"nlm",iter_total=100,variable_time = TRUE,remove_risk_free = TRUE, plot_rates=FALSE,plot_probs=TRUE,gamma_alpha=a,gamma_beta=b)
```
