...
Generally, we would not assume that $\lambda_i = \lambda_j \Rightarrow \pi_i = \pi_j$, but for the purposes of simulating the data in this package we have. This is a simplification, but means that when a PCEG is fit instead of a ZIPCEG, the merging for the leaves will be the same, with the PCEG having lower posterior expected values, as outlined above. We have chosen this simplifying assumption in order to easily compare the PCEG and ZIPCEG models and their performance in relation to the data and each other.  
...
In the following table, we can see each conditional transition probability, informed by the conditional independence statements above. Each probability has its definition alongside its associated stage and situations. We load the *pcegr* package, but also the *ceg* package, as we will use its classes and methods for plotting. Some of the functions in *pcegr* are slightly modified from *ceg*, while others are direct ports, but we have relabeled them so as to not confuse the two. 
```{r}
library(pcegr)
library(ceg)
cov_probs
...
The covariate values being conditioned on uniquely define the stage. For example, when all covariates take a value of 1, this is $l_1$. We can see that incorporating some form of conditional independence essentially merges leaves like $l_3$ and $l_7$, as they are both specified by the shortened covariate combination ($l_3$ has A = 1, $l_7$ has A = 0). 
...
First, we will fit a PCEG model to this data set. This requires specifying an effective sample size, the amount of confidence in the prior. The default choice for CEGs is usually the greatest number of categories a single variable takes, in this case 2, and this is true for PCEGs also. A sensitivity analysis can be carried out on the effects of this choice, but we will not delve into that. 
...
plot(ptree)
```
First, we point out that the nodes corresponding to the Poisson response variable have one outgoing edge labelled $y$. This is because the plotting function for this class requires at least one outgoing edge. In reality, we would not draw the Poisson staged tree with these edges, instead stopping at the leaves. Thus, the situations labeled $s_7$ to $s_{14}$ would instead be the leaves $l_1$ to $l_8$. We also note that that any node that is uncoloured is a singleton stage.

We can see that the model has correctly identified that the individuals of either Age who are Overweight with a History of injury are in the same stage ($s_{10} / l_4$ and $s_{14}/l_8$), and likewise for those with No History ($s_{9}/l_3$ and $s_{13}/l_7$). The model has also correctly identified that History is independent of Weight given Age ($s_{3}$ and $s_{4}$ versus $s_{5}$ and $s_{6}$). 
...
plot(pceg1)
```
There are two graphs plotted, but only the final PCEG on the right hand side should be considered. On the left hand side is an intermediate plot used for propagation analysis, and while not particularly useful, is a legacy of the plotting method for the class.

Similarly to the staged tree, any uncoloured node in the PCEG is a singleton *position*. This is once again a function of the plotting method used, and if the PCEG was drawn by hand, the positions with non-singleton stages ($w_9$ and $w_{10}$) would be coloured with the same colour in the associated Poisson staged tree. 
...
```{r}
total_value_extractor(knee_pain_obs,pmod,level_exclude = 0, zip=FALSE,dec_place=2)
value_extractor(knee_pain_obs,pmod,level_rel_final = 0,true_value = pceg_lambda, zip=FALSE,dec_place=2)
```
We have used the [total_value_extractor()] function for the covariates, but excluded the level for the response in order to compare that to the true value (using the [value_extractor()] function). We can also access the posterior estimates using the S4 class itself, although without as much detail.

```{r}
ptree@posterior.distribution
```
As can be seen, even though the model is misspecified, it correctly identifies the transition probabilities, and the estimates of the rates are close to what we would expect.
...
d<-0.125
```
We will also run the Gibbs sampler for $N=10000$ iterations. When the parameters are estimated using the Gibbs sampler, we will impute the states stochastically. Due to this randomness, and to make sure the results are replicable, we will also set the seed.
```{r}
set.seed(13579)
...
plot(zipceg1)
```
We can see that the model correctly merges the risk states ($s_{9}$ and $s_{13}$, $s_{10}$ and $s_{14}$). We can also see that all of the risk free leaves are in the same stage. Additionally, the rates for the risk leaves are in their expected stages ($s_{20}$ and $s_{28}$ and $s_{22}$ and $s_{30}$). The covariate relationships have also been preserved from the PCEG. 
...
```{r}
zipmod1$lik
chi_sq_calculator(knee_pain_obs,zipmod1,zip=TRUE,dec_place=2)
```
...
ziptree2<-staged.tree.creator(knee_pain_obs,zipmod2,zip=TRUE)
plot(ziptree2)
...
chi_sq_calculator(knee_pain_obs,zipmod1)$chi_sq-chi_sq_calculator(knee_pain_obs,zipmod2)$chi_sq
```
...
We saved the second model as mapmod.alt simply to minimise the rest of the output alongside the graphs, as not saving the model will instead print the output from the model. It will not be used. 
...
```{r}
maptree<-staged.tree.creator(knee_pain_obs,mapmod,zip=TRUE)
plot(maptree)
mapmod$lik
mapmod$lik-zipmod2$lik
chi_sq_calculator(knee_pain_obs,mapmod,zip=TRUE)$chi_sq-chi_sq_calculator(knee_pain_obs,zipmod1,zip=TRUE)$chi_sq
```
As we would expect, the MAP model outperforms both models in terms of Bayes Factor and Chi-square, although only barely in this case.
...
truemod$lik
truetree<-staged.tree.creator(knee_pain,truemod,TRUE,TRUE,zip=FALSE)
plot(truetree)
...
```
Interestingly, when the model is fit to the true data, it incorrectly merges $s_8$ and $s_{11}$, the risk probabilities for leaves 2 and 5, while all of the models fit to the observed data, the "observed" models, after using the parameter estimation methods, correctly identified the risk states. *This is because the observed models are assuming a zero-inflated model, and importantly accounting for the counts, times and the interplay between the rates and the risk probabilities - Not exactly sure that's true, it could be because leaf 5 has a small edge count and imputing the states makes the estimate worse*. For example, when we look at the observed proportion for at risk individuals for those two leaves:
```{r}
summary(dplyr::filter(knee_pain,age=="Young",weight=="Normal",history=="Yes"))
summary(dplyr::filter(knee_pain,age=="Old",weight=="Normal",history=="No"))
v2<-c(285,402);v2/sum(v2)
v5<-c(54,45);v5/sum(v5)
value_extractor(knee_pain_obs,mapmod,level_rel_final = -1,true_value=true_p,dec_place=2)[c(2,5),]
```
We can see that the states have been imputed almost perfectly for leaf 2, but not for the smaller leaf 5. 
...
We will demonstrate the variable discretisation functionality of the *pceg* function in two ways. First, we will fit a Vanilla CEG, then we will fit a PCEG model, in both cases discretising the Age variable. In order to fit a Vanilla CEG, we must first create a suitable data set where the response variable is categorical. The response variable we will choose is simply whether an event occurred or not, so binary. Furthermore, any data set we will analyse is required to have Age as the last covariate, so it must be reordered. Finally, while the Age values in the data set are integers, they must be converted to factors and assumed as categorical in order for the functions to work.
...
We first begin by performing the variable discretisation without any of the inputs described above. We also note that for the purposes of the equivalent sample size, Age is not assumed to be categorical and thus it is still 2. 
...
disc.mod0$result
```
We can see that for the Overweight, Yes History group, it has almost correctly identified the Age break-point of 40. However, the ages before 40 have merged with all of the ages from Over, No and Normal, Yes, while the Normal, No group has only one stage. Hence, because there was no restriction on search length or discretisation between covariate combinations, the result is rather nonsensical for the purposes of discretisation. 
...
disc.mod4$result
```
We can see that once the mergings are equivalent across all four covariate combinations, the model discovers the correct discretisation. Considering the fact that this model is misspecified in the first place, this is an excellent result. We also note that the model was generated assuming the same categorisation across covariate combinations, hence why mirroring performs best. 
...
```{r}
disc.pmod4<-pceg(df,2,poisson_response = TRUE, variable_time=TRUE,gamma_alpha=0.25,gamma_beta=0.1,var_disc=3,disc_length=6,restrict=TRUE,collapse=TRUE)
disc.pmod4$result
```
