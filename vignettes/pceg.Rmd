...
disc.mod3$result
```
We can see that now Over, Yes has 3 categories, rather than the expected 2. This is because the model is unable to join the 1-14 stage and 17-80 stage as it desires, as that would lead to just one stage, breaking the minimum category rule. Meanwhile, the 15-16 stage leads to a decrease in the log marginal likelihood so will not merge. This is an example of how variable discretisation can identify nonlinearity in the covariate effects, while also being able to discover possibly ideal values. Of course, there is no such nonlinearity in this case, and we investigate why this happens.

```{r}
summary(dplyr::filter(df.bin,weight=="Over",history=="Yes",age %in% c(15,16)))
summary(dplyr::filter(df.bin,weight=="Over",history=="Yes",!(age %in% c(15,16))))
3192/3898
```

We can see that due to the small sample size, all 10 observations in the 15-16 age group have suffered knee pain, versus a group proportion of 0.82. This is why the algorithm will not merge this stage with either of the other two.  
