...
```{r}
disc.mod3<-pceg(df.bin,2,var_disc=3,disc_length=2,restrict=TRUE,collapse=TRUE)
disc.mod3$result
```
...
```{r}
disc.mod4<-pceg(df.bin,2,var_disc=3,disc_length=2,restrict=TRUE,cat_limit=2,collapse=TRUE)
disc.mod4$result
```
...
```{r}
disc.mod5<-pceg(df.bin,2,var_disc=3,disc_length=2,restrict=TRUE,mirror=TRUE,cat_limit=2,collapse=TRUE)
disc.mod5$result
```
...
```{r}
disc.tree5<-staged.tree.creator(df.bin,disc.mod5,FALSE,FALSE)
plot(disc.tree5)
disc.ceg5<-sceg(disc.tree5)
plot(disc.ceg5)
```
