...
we will use violin plots to visualise, but there are options to use line plots (default), scatter plots, or histograms. We will use the *nlm* method, due to its lower computational cost per iteration. 
```{r}
mapmod<-zipceg.iter(knee_pain_obs,"nlm",iter_total=100,violin=TRUE,variable_time = TRUE,gamma_alpha=a,gamma_beta=b)$mod
zipceg.iter(knee_pain_obs,"nlm",iter_total=100,violin=TRUE,variable_time = TRUE,plot_rates=FALSE,plot_probs=TRUE,gamma_alpha=a,gamma_beta=b)
```
...
```
We can see that there are some differences between the two models for leaves 1, 3 and 6. We now look at the posterior expected values of the risk probabilities and rates from the MAP model. We also compare these to the true values of $\lambda_i$ and $\pi_i$. Note that the estimates below are for each stage, not leaf. For example, stage 3 contains leaves 3 and 7. The first row for $\lambda$ is the risk free stage, and so the second row is stage 1.  
```{r}
true_lambda<-c(0.5,2.5,3,5,1.5,4,3,5)
true_p<-c(0.2,0.6,0.7,0.9,0.45,0.8,0.7,0.9)
